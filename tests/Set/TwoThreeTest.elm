module Set.TwoThreeTest exposing (tests)

import Basics exposing (..)
import Set.TwoThree as Set exposing (Set)
...
equalSet : Set comparable -> Set comparable -> Expect.Expectation
equalSet left right =
    Expect.equal (Set.toList left) (Set.toList right)


equalTupleOfSets : ( Set comparable, Set comparable ) -> ( Set comparable, Set comparable ) -> Expect.Expectation
equalTupleOfSets ( lFirst, lSecond ) ( rFirst, rSecond ) =
    Expect.equal
        ( (Set.toList lFirst), (Set.toList lSecond) )
        ( (Set.toList rFirst), (Set.toList rSecond) )
...
insertTests =
    [ test "adds new element to empty set" <|
        \() -> equalSet set42 (Set.insert 42 Set.empty)
    , test "adds new element to a set of 100" <|
        \() -> equalSet (Set.fromList (List.range 1 101)) (Set.insert 101 set1To100)
    , test "leaves singleton set intact if it contains given element" <|
        \() -> equalSet set42 (Set.insert 42 set42)
    , test "leaves set of 100 intact if it contains given element" <|
        \() -> equalSet set1To100 (Set.insert 42 set1To100)
    ]
...
removeTests =
    [ test "removes element from singleton set" <|
        \() -> equalSet Set.empty (Set.remove 42 set42)
    , test "removes element from set of 100" <|
        \() -> equalSet (Set.fromList (List.range 1 99)) (Set.remove 100 set1To100)
    , test "leaves singleton set intact if it doesn't contain given element" <|
        \() -> equalSet set42 (Set.remove -1 set42)
    , test "leaves set of 100 intact if it doesn't contain given element" <|
        \() -> equalSet set1To100 (Set.remove -1 set1To100)
    ]
...
foldlTests =
    [ test "with insert and empty set acts as identity function" <|
        \() -> equalSet set1To100 (Set.foldl Set.insert Set.empty set1To100)
...
foldrTests =
    [ test "with insert and empty set acts as identity function" <|
        \() -> equalSet set1To100 (Set.foldr Set.insert Set.empty set1To100)
    , test "with counter and zero acts as size function" <|
...
mapTests =
    [ test "applies given function to singleton element" <|
        \() -> equalSet (Set.singleton 43) (Set.map ((+) 1) set42)
    , test "applies given function to each element" <|
        \() -> equalSet (Set.fromList (List.range -100 -1)) (Set.map negate set1To100)
    ]
...
filterTests =
    [ test "with always True doesn't change anything" <|
        \() -> equalSet set1To100 (Set.filter (always True) set1To100)
    , test "with always False returns empty set" <|
        \() -> equalSet Set.empty (Set.filter (always False) set1To100)
    , test "simple filter" <|
        \() -> equalSet set1To50 (Set.filter isLessThan51 set1To100)
    ]
...
partitionTests =
    [ test "of empty set returns two empty sets" <|
        \() -> equalTupleOfSets ( Set.empty, Set.empty ) (Set.partition isLessThan51 Set.empty)
    , test "simple partition" <|
        \() -> equalTupleOfSets ( set1To50, set51To100 ) (Set.partition isLessThan51 set1To100)
    ]
...
unionTests =
    [ test "with empty set doesn't change anything" <|
        \() -> equalSet set42 (Set.union set42 Set.empty)
    , test "with itself doesn't change anything" <|
        \() -> equalSet set1To100 (Set.union set1To100 set1To100)
    , test "with subset doesn't change anything" <|
        \() -> equalSet set1To100 (Set.union set1To100 set42)
    , test "with superset returns superset" <|
        \() -> equalSet set1To100 (Set.union set42 set1To100)
    , test "contains elements of both singletons" <|
        \() -> equalSet (Set.insert 1 set42) (Set.union set42 (Set.singleton 1))
    , test "consists of elements from either set" <|
        \() ->
            Set.union set1To100 set51To150
                |> equalSet (Set.fromList (List.range 1 150))
    ]
...
intersectTests =
    [ test "with empty set returns empty set" <|
        \() -> equalSet Set.empty (Set.intersect set42 Set.empty)
    , test "with itself doesn't change anything" <|
        \() -> equalSet set1To100 (Set.intersect set1To100 set1To100)
    , test "with subset returns subset" <|
        \() -> equalSet set42 (Set.intersect set1To100 set42)
    , test "with superset doesn't change anything" <|
        \() -> equalSet set42 (Set.intersect set42 set1To100)
    , test "returns empty set given disjunctive sets" <|
        \() -> equalSet Set.empty (Set.intersect set42 (Set.singleton 1))
...
diffTests =
    [ test "with empty set doesn't change anything" <|
        \() -> equalSet set42 (Set.diff set42 Set.empty)
    , test "with itself returns empty set" <|
        \() -> equalSet Set.empty (Set.diff set1To100 set1To100)
    , test "with subset returns set without subset elements" <|
        \() -> equalSet (Set.remove 42 set1To100) (Set.diff set1To100 set42)
    , test "with superset returns empty set" <|
        \() -> equalSet Set.empty (Set.diff set42 set1To100)
    , test "doesn't change anything given disjunctive sets" <|
        \() -> equalSet set42 (Set.diff set42 (Set.singleton 1))
...
fromListTests =
    [ test "returns empty set for empty list" <|
        \() -> equalSet Set.empty (Set.fromList [])
    , test "returns singleton set for singleton list" <|
        \() -> equalSet set42 (Set.fromList [ 42 ])
    , test "returns set with unique list elements" <|
        \() -> equalSet set1To100 (Set.fromList (1 :: (List.range 1 100)))
    ]
